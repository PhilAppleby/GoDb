...
# Parse out the header record.
        ...
        hdrData.extend((varid, varid + "_prob",  varid + "_plat", varid + "_min"))
        for i, str_geno in enumerate(sfx):
          if str_geno != ".":
            ...
            if options.platform != None:
              sampleDict[colPosns[i]].extend((str(i_call), str(max_prob), options.platform, hdr_allele))
            else:
              sampleDict[colPosns[i]].extend((str(i_call), str(max_prob), assay_expand[geno[probidx+1]], hdr_allele))
          else:
